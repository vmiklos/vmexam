...
int main(int argc, char** argv)
{
    ...
    if (clang_Cursor_isNull(aCanonicalCursor) ||
        ...
        return 1; // not sure about this, perhaps could use the referenced
                  // cursor in this case.

    CXSourceLocation aDeclarationLocation =
        clang_getCursorLocation(aCanonicalCursor);
    CXFile aDeclarationFile;
    unsigned nDeclarationLine;
    unsigned nDeclarationCol;
    clang_getExpansionLocation(aDeclarationLocation, &aDeclarationFile,
                               &nDeclarationLine, &nDeclarationCol, nullptr);
    CXString aDeclarationFileName = clang_getFileName(aDeclarationFile);
    std::cerr << "aDeclarationFileName is '"
              << clang_getCString(aDeclarationFileName) << "'" << std::endl;
    clang_disposeString(aDeclarationFileName);
    std::cerr << "nDeclarationLine is " << nDeclarationLine
              << ", nDeclarationCol is " << nDeclarationCol << std::endl;
