...
NSData* queryNominatim(NSString* query)
{
    ...
    NSString* queryEncoded =
        [query stringByAddingPercentEncodingWithAllowedCharacters:
                   [NSCharacterSet URLHostAllowedCharacterSet]];
    NSString* url =
        [urlPrefix stringByAppendingFormat:@"q=%@&format=json", queryEncoded];
...
void osmify(NSString* query)
{
    ...
    NSString* housenumber =
        [[element objectForKey:@"tags"] objectForKey:@"addr:housenumber"];
    NSString* postcode =
        [[element objectForKey:@"tags"] objectForKey:@"addr:postcode"];
    NSString* street =
        [[element objectForKey:@"tags"] objectForKey:@"addr:street"];
...
int main(int argc, const char* argv[])
{
    @autoreleasepool
    {
        ...
        {
            printf("%s\n", [@"usage: addr-osmify <query>" UTF8String]);
            printf("\n");
